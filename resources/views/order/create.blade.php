                            ...
                            <div class="tab-pane fade show @if(!$key) active @endif" id="{{str_replace(' ','_',$category)}}_conteiner" role="tabpanel" aria-labelledby="{{$category}}-tab">
                              ...
                              @foreach($products as $key => $product)
                                @if($product->category == $category)
                                  ...
                                  <div class="col-md-3 ">
                                    <div class="product-conteiner option_add_product" id ="{{$product->id}}_{{$product->store_id}}_{{$product->name}}"  style = "background-image: url({{url('users/'.\Auth::user()->myAdmin(Auth::user()->rel_store_id).'/products/'.$product->image1)}});">
